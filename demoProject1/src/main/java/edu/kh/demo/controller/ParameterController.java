...
//Controller : 요청, 응답, 제어 역활
//Bean       : Spring이 만든 객체


@Slf4j
@Controller // 컨트롤러임을 명시 + Bean 등록
@RequestMapping("param") // parma으로 시작하는 요청을 해당 컨트롤에 맵핑한다.
												// 해당 Controller에 맵핑.
public class ParameterController {

	
	@RequestMapping("main") //  / Param/main 요청을 ㅁ처리하는 메서드
	...
	// test1 GET 방식 요청을 처리하는 메서드
	//1.HttpServletRequest.getParameter("key") 사용하기
	
	//HttpSErvletRequest
	// - 요청한 클라이언트의 정보, 제출된 파라미터등 을
	// 저장한 객체
	@GetMapping("test1")
	public String test1(HttpServletRequest req) {
		...
		// -> 가능!!
		//            왜 ??
		// 							Spring의 ArgumentResolver 떄문에 가능
		// ArgumentResolver(전달 인자 해결사)
		// -> Controller 메서드에 작성된 매개변수 
		// 타입에  맞는 객체가 존재하면 바인딩
		//없으면 생성해서 바인딩.
...
//		System.out.println(inputAddress);
		...
		// -위임 시 HttpServletRequest, HttpServlerResponse를
		// 같이 위임.
  ...
  - 낱개(단수, 하나, 독립된) 파라미터 컨트롤러에 얻는 방법 */
	...
	public String test2(
			...
			@RequestParam("price") String ps
			) {
		
		//파라미터 중 prive를 int로 저장할 수 있는 이유
		// -> HTML에서 얻어오는 모든값은 기본적이러 Sting 형태이나
		...
		/*log를 이용해서 파라미터 출력하기
		 * 
		 * 1) 클래스위에 @Slf4j 어노테이션 추가 (Lombok) 제공
		 * 2) log.debug 또는 log.info 등을 이영해서 log 출력
		 * */
	...
	public String test3(
		...
		return "param/result3"; // Forward 하겠다.
	}
	...
	public String test4(
		...
		/*	type="text" 관련 input태그는 
		 * 값을 작성하지 않은경우 빈칸("") 제출된다.
		 * -> 제출되는 값이 존재하기 떄문에 
		 * 	@RequestParam()에 아무 속성을 적지 않아도 잘 얻어와진다!!! 
		 * 
		 * name 속성 == values 속성 (같은것!!)
		 * 
		 * *required : 필수
		 * -> 기본 값이 true
		 * -> true가 설정된 경우 == 꼭 제출 되어야하는 파라미터
		 * 		--> 만약 제출되지 않을 경우
		 * 				HTT{ 상태코드 400 -> 400 Bad Request 에러 발생.
		 * 
		 * * checkbox, radio : 체크 되지않으면 제출 X 400 Erorr
		 * 
		 * 직접 쿼리스트링을 작성하는데
		 * 	key=valye가 누락된 경우 -> 400에러
		 * 
		 * 
		 * * required = false일 떄, 값이 제출되지 않으면 -> null
		 * 
		 * 
		 * * defaultValue :
	...
	//많은 양의 파라미터를 하나의 dto로 한번에 저장할 수 있어 굉장히 편함

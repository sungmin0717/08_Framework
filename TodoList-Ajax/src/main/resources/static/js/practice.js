...
/* todoNo 일치하는 할 일의 모든 정보(다수) 얻어오기 */
...
searchBtn2.addEventListener("click", () => {
  ...
  // (비동기) GET 방식 요청은 주소에 전달할 값 (파라미터)를 
  // 쿼리스트링 형태로 작성!!!

  const todoNo = inputTodoNo2.value; // inputTodoNo2 값 가져오기 (입력된 할 일 번호 얻어오기)

  fetch("/todo/todoDetail?todoNo=" + todoNo)
  .then(response => {
    if(response.ok){
      return response.text(); //응답 결과를 text(string)으로 변환
    }

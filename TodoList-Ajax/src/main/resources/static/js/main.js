...
changeComplete.addEventListener("click", () => {
  ...
  const todoComplete 
    = document.querySelector("#popupComplete").innerText;
...
deleteBtn.addEventListener("click", () => {
  ...
  // 비동기로 삭제 요청하기(DELECT 방식 요청)
  fetch("/todo/todoDelete", {
    method  : "DELETE", // DELECT 방식 요청
    headers : {"Content-Type" : "application/json"},
    body    : todoNo 
  })
.then(response => {
...
updateView.addEventListener("click", () => {
  ...
  updateLayer.classList.remove("popup-hidden"); // 수정 레이어 버이기
...
updateBtn.addEventListener("click", () => {
  ...
  obj.todoNo = updateBtn.dataset.todoNo;  // 그대로 dataset
  //버튼에 dataset 값 얻어오기.

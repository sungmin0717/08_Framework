...
addBtn.addEventListener("click", () => {
  ...
  // - body : POST/PUT/DELETE 요청시 전달할 데이터
  ...
  // response.text() : 컨트롤러 반환 값을 text형태로 반환해서
                        // 아래 두번쨰 then 매개 변수로 전달
  .then(result => {
    console.log("result : ", result);

    //비동기 통신 응답 결과가 1인경우 (삽입 성공인 경우)
    if(result > 0){
      alert(" 야 야 성공해써");
...
searchBtn.addEventListener("click", () => {
  ...
  const todoNo = inputTodoNo.value; // todoNo에 값 얻어오기

...
const getAirQuality = async (cityName) => {
  //발급받은 api 인코딩
  // 요청 url 
  // 요청 주소 조합 비동기 post 방식으로 요청해서 가져오기 (숨기기)
...
/* 
  ...
  -이 키워드가 붙은 함수는 항상 Promise객체를 반환함
  ...
  await : 비동기 요청을 동기식 요청으로 변환
    ...
    - await키워드가 붙은 함수가 종료 될 때 가지 코드 해석을 일시중지
      (동기식)
*/
...
// 응답이 돌아어면 reponse1 변수에 응답결과 (첫 번째 then의 response)가 저장됨
  ...
  try{
    ...
    // 이모지/등급 배열
    const greadeEmoji = ['😄', '🙂', '😷', '🤢'];
    const greadeText = ['좋음', '보통', '나쁨', '매우나쁨'];


    pm10Grade.innerText = greadeEmoji[item.pm10Grade - 1];
    pm10GradeText.innerText = greadeText[item.pm10Grade - 1];
    pm10Value.innerText = `미세먼지 농도 : ${item['pm10Value']} ㎍/㎥`;


    pm25Grade.innerText = greadeEmoji[item.pm25Grade - 1];
    pm25GradeText.innerText = greadeText[item.pm25Grade - 1];
...
//java 부분 sleect 변경 후 조회 클릭 시에 대한 동작

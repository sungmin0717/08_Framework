...
//    제공 샘플과 다른 부분
    ...
    /*제공 샘플과 다른 부분*/
    // - 샘플 코드 수행 시 응답 데이터에서 한글이 깨는 문제 발생
    // -> chrset-UTF-8 을추가하여 응답에티ㅓ의 문자 인코딩을 UTF-8로 지정
    // -> 한글 깨짐현상
    ...
    log.debug(response);
			...
			// -> JSON데이터를 Java 객체(Map, DTO)로 변환할 수 있는 라이브러리
			// spring =-starter-web dependency에 Jackson data-bind포함되어있음
			
			
			//JSON을 쉽게 다루거나 변활할 수 있는 객체

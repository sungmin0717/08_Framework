 ...
 * 
 * Cloent <->Filter <-> dispatcher Servlet <-> Interceptor <-> Controller ..... 
 * 
 * 
 ...
 * 
 * 2) postHandle()
 * Comtoller -> Dispathfher Servlett
 *  사이에 요청 / 응답을 가로채서 수행
 * 
 * 3) afterCompletion()
 * View Resolver 0< Dispathchar Servlet 사이 수행
 */
@Slf4j
...
//	log.info("------ BoardTypeInterce"tor 전처리 메서드 실행 -------");	
		
		// 어떤 요청이 와도 
		//header에 출력되는 게시판 메뉴를 얻어와 DB에서 
		// Applocation Scope에 세팅
		
		// 1) Applocation scope 객체 얻어오기

...
public class BoardServiceImpl implements BoardService{
	...
	public Map<String, Object> selectBoardList(int boardCode, int cp) {
		...
		// 		조회될 목록 페이지. 출력할 페이지네이션을 값을 계산할
		// 		Pagenation 객체  생성하기
		
		Pagination pagination = new Pagination(cp, listCount);
		
		// 3 DB에서 cp (조회 하려는 페이지)에 해당하느 행을 조회 
		// ex) cp == 1 , 전체 목록 중 1~ 10 행 결과만  반환
		// ex) cp == 2 , 전체 목록 중 11 ~ 20 행 결과만  반환
		// ex) cp == 10 , 전체 목록 중 99 ~ 100 행 결과만  반환
		
		/*[RowBound 객체]
		 * - Mybatis 제공 객체
		 * 
		 * - 지정된크리 (offset) 만큼 행을 건너 뛰고
		 * 		제한된 크기 (limit) 칸믕의 행을 조회함
		 * 
		...
		List<Board> boardList = mapper.selectBoardList(boardCode, rowBound);
...
//			= Map.of("boardList", boardList, "pagination", pagination);
	...
	@Override
	public Map<String, Object> boardNo(int boardNo, int memberNo) {
		...
		// 2. 좋아요 여부에 따라 INSERT/DELECT Mapper 호출
		int result2 = 0;
		if(result == 0) {
		...
		if(result2 > 0) {
			...
			return null; // INSERT, DELECT 실패 시 
		}

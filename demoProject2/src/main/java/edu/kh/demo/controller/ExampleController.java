...
public class ExampleController {
	 ...
	 */
	 ...
	 public String ex1(HttpServletRequest req, Model model) {
		 ...
		 //값이 초기화된 Student . build
		 //값을 이어쓴다라고해서 메서드 체이닝.
		 Student std = Student.builder()
				 									.studentNo("1111")
				 									.name			("짱구")
				 									.age			(15)
				 									.build(); // -> 필드 전체가 아닌 일부 초기화.
	 ...
	 /** model : Spring에서 데이터를 전달하는 용도의 객체.
	  * 							(기본 scope : request)
	  * @param model
	  * @return
	  */
	 ...
	 //주소중 값으 얻어오고 싶은 부분을
	 // {} 작성하고 {}를 지칭하는 이름(변수명) 내무에 작성
	 @GetMapping("ex3/{number}")
	 public String pathVariableTest(
			 @PathVariable("number") int num
			 // 주소중 {number} 자리에 작성된 값을 얻어와
			 //매개 변수 int num에 저장하겠다.
			 ){
		...
		public String ex4(Model model) {
			...
			Student std = new Student("1", "맹구", 5, null);
